...
var defaultOppX = [];
var defaultPlayX = [];
public var data:Map<Int, {lastNote:{time:Float, id:Int}}> = [];
    ...
    if (Options.gameplayShaders){
        ...
        if (distorShit > 0) {// turn off the shader when it's not the type of note
            ...
            if(distorShit <= 0) {
                ...
                for (boyfriends in strumLines.members[1].characters){
                    if (boyfriends.shader == distorDad || boyfriends.shader == glitches)boyfriends.shader = null;
                }
            ...
            if(doShits && doDoubles){
                if (note.note.isSustainNote) return;
                    if (FlxG.random.float(0, 1) < 0.5) {
                        ...
                        if (boyfriend.curCharacter == "darwinfwglitch"){
...
function onPlayerHit(note:NoteHitnote){
    var curNotes = note.noteType;
    ...
    if (curNotes == "Glitch Note"){
        ...
        if (Options.gameplayShaders && doGlitchEffect){
            ...
            for (boyfriends in strumLines.members[1].characters){
                if (boyfriends.shader == null || boyfriends.shader == glitches)boyfriends.shader = distorDad;
            }
    ...
    if (!glitching && Options.gameplayShaders && doGlitchEffect){
        ...
        distorDad.negativity = 0;
            ...
            for (bfs in strumLines.members[1].characters){
                    if (bfs.shader == distorDad || bfs.shader == glitches)bfs.shader = null;
                }
            } 
...
function gfGlitch(){
    ...
    if (Options.gameplayShaders && doGlitchEffect){
        ...
        for (gfs in strumLines.members[2].characters){
            if (gfs.shader == null || gfs.shader == glitches)gfs.shader = distorDad;
        }

            distorDad.negativity = FlxG.random.float(1.0, 2.0);
        }
        if (!glitching && Options.gameplayShaders && doGlitchEffect){
            distorDad.binaryIntensity = 0;
            distorDad.negativity = 0;
...
function gfGlitchend(){
    for (gfs in strumLines.members[2].characters){
        if (gfs.shader == distorDad || gfs.shader == glitches)gfs.shader = null;
    }
}