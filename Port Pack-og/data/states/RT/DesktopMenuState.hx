...
import flixel.FlxCamera;
    ...
    function addSets(cock:FlxSprite, i) {
        cock.frames = Paths.getSparrowAtlas('RT stuff/userinterface/desktop/bgLayers');
        ...
        introTmr = new FlxTimer().start(1.5, function(yeah:FlxTimer)
        {
            ...
            introTmr = new FlxTimer().start(1.5, function(man:FlxTimer)
            {
                ...
                introTmr = new FlxTimer().start(0.5, function(among:FlxTimer)
                {
                    ...
                    FlxG.sound.play(Paths.sound('RT/xp/windowsXP'), 0.6);
            ...
            switch (menuItems[curSelected]) {
                case 'play':
                    ...
                    FlxTween.tween(FlxG.camera, {zoom: 1}, 1, {startDelay: 1, ease: FlxEase.quintInOut, onComplete: Void -> {
                     MusicBeatState.skipTransIn = MusicBeatState.skipTransOut = true;
                        FlxG.switchState(new ModState("RT/DesktopSongSelectState"));
                    }});		
                ...
                case 'gallery':
                    ...
                    FlxTween.tween(FlxG.camera, {zoom: 2.5}, time, {startDelay: 1, ease: FlxEase.cubeIn, onComplete: Void -> {
                        new FlxTimer().start(0.7, Void -> {FlxG.switchState(new ModState("RT/DesktopCreditsState"));});
                ...
                case 'options':
                    ...
                    FlxTween.tween(FlxG.camera, {zoom: 1}, 1, {startDelay: 1, ease: FlxEase.quintInOut, onComplete: Void -> {
                     MusicBeatState.skipTransIn = MusicBeatState.skipTransOut = true;
                        FlxG.switchState(new ModState("RT/DesktopOptionsState"));
                    }});		
                    dimEverythingButTheMonitor();
                case 'credits':
                    trace("reddits");
                    FlxTween.tween(FlxG.camera, {zoom: 1}, 1, {startDelay: 1, ease: FlxEase.quintInOut, onComplete: Void -> {
                     MusicBeatState.skipTransIn = MusicBeatState.skipTransOut = true;
                        FlxG.switchState(new ModState("RT/DesktopCreditsState"));
                    }});		
                    dimEverythingButTheMonitor();
            }
